...
def detect_security_group_drifts(
    ...
    for m in matches:
        ...
        # TAG DRIFT DISABLED — tags should NOT cause drift
        tag_drift = False
        tag_diff_count = 0


        # DESCRIPTION DRIFT (optional)
        description_drift = (
            normalize_text(group_description(source))
            != normalize_text(group_description(target))
        )

        # TAG DRIFT DISABLED — tags do NOT cause drift
        tag_drift = False
        tag_diff_count = 0

        # Only missing rules (and optional description drift) count as drift
        if missing_ingress or missing_egress:
            drifts.append(
                SgDrift(
                    ...
                    extra_ingress=[],   # always empty
                    missing_egress=missing_egress,
                    extra_egress=[],     # always empty
                    tag_drift=False,
                    tag_diff_count=0,
